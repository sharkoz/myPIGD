app.factory('DataSource', ['$http',function($http){
  ...
  return {
    ...
    get: function(callback,errorcallback, path){
      $http
          .get('https://build.phonegap.com'+path, {timeout:17000})
 ...
 app.factory('AuthToken', function($localStorage, DataSource){
  ...
  return {
    setApps : function(apps){
      comp = function(el, index, liste){
        keys = _.keys(el);
        if(!_.isEqual(el, _.pick($localStorage.apps.apps[index],keys)))
        {
          diff = diff + 1;
          return 1;
        }
